...
fn main() {
    ...
    let window = WindowBuilder::new()
        .with_title("Example 04: multiple windows")
    ...
    let other_window = WindowBuilder::new()
        .with_title("Example 04: multiple windows (Debug)")
        .with_inner_size(winit::dpi::LogicalSize::new(f64::from(window_width), f64::from(window_height)))
        .build(&event_loop)
        .unwrap();
    ...
    let mut other_renderer = Renderer::<Vertex, _>::new(
        &other_window,
        window_width,
        window_height,
        [
    ...
    other_renderer.set_vertex_shader(include_bytes!("./shader/vert_basic.spv"));
    other_renderer.set_fragment_shader(include_bytes!("./shader/frag.spv"));
    ...
    renderer.clear_color = [0.30, 0.75, 0.90, 1.0];
    other_renderer.clear_color = [0.30, 0.75, 0.90, 1.0];
            ...
            match event {
                ...
                Event::MainEventsCleared => {
                    ...
                    other_renderer.render(
                        DrawUniform { view_mat: camera_transform.get_inverse_matrix(), proj_mat, light_dir, ambient_light: 0.1 },
                        (0..(2 * WORM_BONE_COUNT)).map(|_| cube_meshi).collect(),
                        {
